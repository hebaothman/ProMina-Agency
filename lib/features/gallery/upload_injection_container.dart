...
Future<void> uploadDependencyInjectionInit() async {
  ...
  // Blocs
  sl.registerFactory<UploadCubit>(() => UploadCubit(uploadUseCase: sl(), getImagesUseCase: sl()));

  // Use cases
  //We use lazy we don't need to load the whole app
  sl.registerLazySingleton<UploadUseCase>(() => UploadUseCase(uploadRepository: sl()));
  sl.registerLazySingleton<GetImagesUseCase>(() => GetImagesUseCase(uploadRepository: sl()));
